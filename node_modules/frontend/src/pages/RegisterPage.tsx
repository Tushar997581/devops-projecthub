    ...
    <section className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
        <div className="flex gap-3">
          <input className="w-full rounded-lg border border-slate-300 px-4 py-2" placeholder="First name" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} required />
          <input className="w-full rounded-lg border border-slate-300 px-4 py-2" placeholder="Last name" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} required />
        </div>
        ...
        <button className="w-full rounded-lg bg-slate-900 px-4 py-2 font-medium text-white" type="submit">Create account</button>
      </form>
